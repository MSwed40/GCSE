...
export default function Quiz({ subject, theme, onBack }) {
  ...
  if (!allQuestions.length) {
    return (<div style={{ background:theme, minHeight:'100vh', padding:'2rem', fontFamily:'sans-serif' }}>
  ...
  return (<div style={{ background:theme, minHeight:'100vh', padding:'2rem', fontFamily:'sans-serif' }}>
    <button onClick={onBack} style={{ marginBottom:'1rem' }}>⬅ Back</button>
    <h2 style={{ marginTop:0 }}>{subject} Quiz</h2>
    <div style={{ background:'#fff', borderRadius:12, padding:'1.25rem', boxShadow:'0 2px 8px rgba(0,0,0,0.1)', maxWidth:720 }}>
      <div style={{ marginBottom:'0.5rem', color:'#6b7280' }}><small>Topic: {q.topic} • {q.subtopic} • Difficulty: {q.difficulty}</small></div>
      <h3 style={{ marginTop:0 }}>{q.question}</h3>
      <div style={{ marginTop:'1rem' }}>
        {q.choices.map((c, idx) => (
          <button key={idx} onClick={()=>handleAnswer(idx)}
            style={{ display:'block', width:'100%', textAlign:'left', margin:'0.5rem 0', padding:'0.75rem 1rem', borderRadius:10, border:'1px solid #e5e7eb',
                     background: selected===null ? '#f9fafb' : (idx===q.answer ? '#dcfce7' : (selected===idx ? '#fee2e2' : '#f9fafb')) }}>
            <b>{String.fromCharCode(65+idx)}.</b> {c}
          </button>
        ))}
      </div>
      <p style={{ marginTop:'0.75rem', fontWeight:'bold' }}>Score: {score}/{current+1} ({((score/(current+1))*100).toFixed(1)}%)</p>
    </div>
  </div>)
}
